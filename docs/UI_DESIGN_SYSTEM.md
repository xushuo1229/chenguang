...
- **B · Notion/Linear 信息系统**：卡片化、模块分区、Dashboard 式组织，结构即信息。
...
3. 语义色不得混用：teal=完成、rose=错误、sky=信息、iris=AI。
...
单一字族（跟随系统 CJK 栈）：`--font` = HarmonyOS Sans SC / MiSans / PingFang SC / Microsoft YaHei UI / Segoe UI / system-ui。
...
| 类型 | 规格 |
|---|---|
| 入场 reveal | fade + translateY(24px)，320ms `--ease`，仅一次，IntersectionObserver 触发 |
| hover | translateY(‑2px) + 阴影加深 + 边框提亮，≤200ms |
| 弹窗 | fade + scale(0.97→1)，200ms |
| 数字 | hero 统计数字 count-up（1500ms，一次性） |
...
### 卡片（B · 信息系统核心）
- 底 `--bg-card`，1px `--border-soft` 边框，`--r-lg`，padding 24–28px。
- 结构固定：图标/指标区 → 标题 → 描述 → 数据行（`--font-mono` 数字）→ 动作。
- 悬浮：抬升 2px + 阴影 + 边框提亮；**禁止**背景变色闪烁。
- 功能语义色（amber/teal/sky/rose）只出现在图标底色与数据徽章，不整卡染色。

### 表单（登录/注册）
- 输入框：`--bg-input` 底、1px `--border-strong`、`--r-sm`、**16px 字号**、padding 13px 14px。
- 聚焦：琥珀 3px 柔和焦点环（`--brand-amber-weak`）；错误：rose 边框 + `hint.err` 文案。
...
- 容器 max-width 1080px（Landing 内容）/ 1200px（顶栏），左右 padding 20–24px。
...
## 9. 内部页面组件（Phase UI-2 扩展）

> 适用范围：workbench / stats / goals / ai 四个内部页。共享组件统一放在
...
- 结构：三个 12px 标签（点 + 字），中间 4px 轨道带刻度线；当前阶段 `is-current`（amber 加粗），
  已过阶段 `is-done`（teal）。纯装饰，`aria-hidden="true"`（数值语义由 progressbar 表达）。
...
- 语义色只落在图标 chip（`.card-ico` 26px 圆角块）、徽章与进度条上。
...
### 按钮圆角（内部页补遗）
- 一切按钮（顶栏链接按钮、btn-wb、modal-foot、btn-new、btn-refresh）统一 `--r-btn: 14px`；
  唯一例外是状态徽章 / 小 chip，继续用 `--r-pill`。
