...
当前 Growth Intelligence 已具备基础趋势检测（7/30/90 天环比）、成长评分、领域状态和信号生成。Phase 21 的 Timeline/Narrative/Retention 层已覆盖"用户如何感知成长"。

核心缺口：系统无法区分**一次提升**、**短期波动**、**稳定趋势**和**长期能力形成**四个不同层级。`buildTrend()` 将所有 "rising" 视为相同信号，不区分置信度和持续性。

判定：**READY FOR IMPLEMENT**（Phase 22.1 应先建立统一 Growth Signal 结构，再扩展深度理解）。
...
| 哪些变化值得反馈 | ✅ 可回答 | `trendState.importantChanges`（threshold ≥ 20%） |
...
1. **统一 Growth Signal 结构**：当前 `positiveSignals` 和 `riskSignals` 是 ad-hoc 对象（`{type, severity, reason, evidence}`），无 `strength`、`confidence`、`direction` 标准字段。
2. **行为变化 vs 波动区分**：`buildTrend()` 的 `status = rising` 阈值是 `delta >= 20%`，但未校验持续性（是否连续多天递增）或波动率（volatility < 阈值才可信）。
3. **长期习惯形成检测**：`focus_habit_forming` 类型存在但仅基于 30 天 trend = rising，无"连续 N 天 + 波动率低 + 频率高"的复合条件。
4. **Opportunity Detection**：positiveSignals 可检测"focus rising"，但不会主动建议"考虑建立阅读目标"。
...
| Signal | 来源 | 结构 |
|---|---|---|
| focus | `Analytics.getTrend('focus')` | `{metric, label, span, current, previous, delta, status, volatility, evidence}` |
| exercise | `Analytics.getTrend('exerciseMinutes')` | 同上 |
| reading | `Analytics.getTrend('pages')` | 同上 |
| english | `Analytics.getTrend('english')` | 同上 |
| todo | `Analytics.getTrend('todoDone')` | 同上 |
| course | `Analytics.getCourseSummary()` | `{total, doing, avgProgress, lowProgressCount}` |
| goals | `GoalEngine.computeGoalsProgress()` | `{active, completed, expired, atRisk, risk[]}` |
| streak | `Analytics.getStreaks()` | `{currentStreak, longestStreak, lastDate, todayDone}` |
...
| 缺失 | 用途 | 建议 |
|---|---|---|
| `type` | 区分 behavior_change / consistency / stage_transition / identity | 从 status + volatility + activeDays 推导 |
| `direction` | 统一 up / down / flat（替代 status 字符串） | 从 delta 正负号直接映射 |
| `strength` | 0-1 信号强度（考虑 delta 幅度 + volatility 反比 + activeDays） | `abs(delta) / (100 + volatility)` 归一化 |
| `confidence` | 0-1 信号可信度 | 基于 activeDays 数量和 volatility |
| `isSustained` | 是否为稳定趋势（非一次波动） | `volatility < 50 && activeDays >= 5` |

### 重复计算检查

- ✅ `stateCache` 已存在（基于 `_meta.revision` 缓存）
- ✅ DailyFeedback / Timeline / RetentionContext 均消费已有 `growthState`
...
- ✅ `DailyFeedback.buildHighlights()` 接收 `growthState`，不直接调用 Analytics
- ✅ `GrowthTimeline.buildTimeline()` 消费 `growthState`
- ⚠️ `pages/workbench.js` 中 `renderGrowthBrief()` 的 `changesEl` 使用 `change.label + '：' + change.current + ...` 直接拼接技术值展示（未经过用户语言格式化层）
...
| 维度 | 评估 |
|---|---|
| **收益** | AI 可获得结构化信号（direction + confidence）替代从 trends 中自行推断方向 |
| **成本** | 新增 token 开销（约 200-400 tokens，视信号数量） |
| **Token 影响** | 中等（当前 context 已较大，growthState 占比高） |
| **必要性** | **不推荐当前阶段增加**。`ctx.growth.trends` 已包含 per-metric trend 数据，AI 可从中推断方向。增加独立 `growthSignals` 会导致数据冗余 |
...
| `dailyFeedback.js` | ❌ | ✅ 正确 |
| `growthMemory.js` | ✅（confirm/reject） | ✅ 正确（用户确认后才写入） |
| `aiContext.js` | ❌ | ✅ 正确 |
...
| 模块 | p50 (ms) | p95 (ms) | 目标 | 判定 |
|---|---|---|---|---|
| GrowthIntelligence.buildDailyInsight | 34.67 | 38.13 | <200ms | ✅ |
| DailyFeedback | 0.012 | 0.039 | <10ms | ✅ |
| GrowthTimeline | 0.013 | 0.069 | <10ms | ✅ |
| GrowthNarrative | 0.011 | 0.134 | <10ms | ✅ |
| RetentionContext.buildWelcomeBack | 0.352 | 0.496 | <10ms | ✅ |
| RetentionContext.buildStreakReminder | 0.361 | 0.555 | <10ms | ✅ |
| RetentionContext.getRetentionCandidate | 0.002 | 0.007 | <10ms | ✅ |
...
| token / password / API key 泄露 | ❌ 无 | `rg password|token|api.?key` 0 匹配 |
...
### Phase 22.1 · Unified Growth Signal Structure
...
### Phase 22.2 · Habit Formation Detection
...
### Phase 22.3 · Opportunity Detection
...
| 风险 | 级别 | 建议 |
|---|---|---|
| Growth Signal 无统一 confidence/strength | P0 | Phase 22.1 解决 |
| 无法区分一次提升 vs 稳定趋势 | P0 | Phase 22.1/22.2 解决 |
| 无主动 Opportunity Detection | P1 | Phase 22.3 解决 |
| Workbench changesEl 直接展示技术值 | P1 | Phase 22.1 一并优化 |
| GrowthIntelligence p95 = 38ms | P2 | 当前可接受，如新增信号结构需关注性能 |
