...
| 声称 | 代码验证 | 结果 |
|---|---|---|
| 新增 `js/habitFormation.js` | ✅ 文件存在 | 通过 |
| Model B 公式 | ✅ 代码一致 | 通过 |
| 阈值 `habitScore >= 0.5 && freq >= 0.3 && maxConsec >= 3` | ✅ 代码一致 | 通过 |
| `minSamples = 3` | ✅ 代码一致（提前返回） | 通过 |
| 纯函数，无副作用 | ✅ 零导入 | 通过 |
| 不修改已有模块 | ✅ git 差异仅新增文件 | 通过 |
...
`activeDays >= 3` 通过早期返回实现（`if (activeDays < THRESHOLDS.minSamples) return`），而不是在 `isHabitForming` 条件中显式检查。功能等价。
...
- If the caller passes in `windowDays=30` but only provides 7 days of data, the frequency is still calculated as 30.
...
- 只计算活跃日的值（排除零值日）
- 例如 `[25, 25, 25, 0, 0, 0, 0]` 的一致性 = 1.0（3 个活跃日值全部相同）

## 6.2 CV 边界情况

| 场景 | 结果 | 是否安全 |
|---|---|---|
| mean = 0（不可能：nonZero 值 > 0） | 不适用 | ✅ |
| mean 极低（全 1） | cv = 0, 一致性 = 1.0 | ✅ 无 NaN/Inf |
| 极端值 (1, 1000) | cv ≈ 1.414, 一致性 = 0.0 | ✅ |
...
| 模式 | 频率 | 一致性 | 最大连续 | 得分 | 形成 | 语义评估 |
|---|---|---|---|---|---|---|
| 1111111 | 1.00 | 1.00 | 7 | 1.00 | true | ✅ 习惯 |
| 1010101 | 0.57 | 1.00 | 1 | 0.55 | false | ✅ 频率够但无连续性 |
| 1111000 | 0.57 | 1.00 | 4 | 0.70 | **true** | ⚠️ 4 天即判定形成 |
| 1000001 | 提前返回 | - | - | 0.00 | false | ✅ minSamples 保护 |
| 0011111 | 0.71 | 1.00 | 5 | 0.80 | **true** | ⚠️ 5 天判定形成 |

* *关键发现：** consistency = 1.0 对所有“活跃日值相同”的模式成立，无论活跃日是否连续。consistency 仅衡量“活跃日的值稳定度”，不衡量“行为出现的规律性”。这是设计选择，但可能导致 consistency 因子在二值场景下失去区分度。
...
| 模式 | 最大连续 | 分数 | 形成 |
|---|---|---|---|
| F: 111111000...（旧习惯，已停） | 6 | 0.440 | 假 |
| G: 000...111111（新习惯，刚开始） | 6 | 0.440 | 假 |
| 1111110（7天窗口，旧 streak） | 6 | 0.900 | 真 |
| 0111111（7天窗口，新 streak） | 6 | 0.900 | 真 |
...
| Scene | activeDays | Result | Evaluation |
|---|---|---|---|
|0 天 |0 |错误 |✅ |
|1 天 |1 |false |✅ minSamples 阻止 |
|2 天 |2 |false |✅ minSamples 阻止 |
|3 天连续 (111) |3 |**真实**（分数=1.0） |⚠️ 过于激进 |
|3 天分散 |3 |假 |✅ maxConsec < 3 |

* *`111` 在 3D 窗口中立即判定为 `isHabitForming = true`。**

产品语义：3天的行为更准确的描述应该是“早期形成信号”，而不是“习惯形成”。3天是最低门槛，容易误判短期尝试为习惯形成。

* *P1 风险：建议将 `maxConsecutive` 门槛提升至 5 天，或将输出增加 `stage` 字段（早期/形成中/已建立）。**
...
| 单峰 | 1 天 | false | ✅ |
| 2 天 | 2 天 | false | ✅ |
| 短期趋势（高波动值） | [10,30,50,0,0,0,0] | false | ✅ 一致性低 |
| 交替模式 | 1010101 | false | ✅ 最大连续=1 |
| 分散活跃 | 110110...（最大连续=2） | false | ✅ |
| **4 天连续** | 1111000 | **true** | ⚠️ 偏激进 |
| **极端值** | [1,1000,1,1000,...] | **true** | ⚠️ 一致性=0 但 频率=1.0, 连续性=1.0 → 分数=0.70 |
| 长期中断后恢复 | 000...111111（6天） | false | ✅ 频率=0.2 < 0.3 |

* *两个假阳性风险：**
1. 4 天连续即触发 forming（P1，应提高门槛）
2. 极端值波动（1 对 1000）时 consistency=0 但 score 仍 ≥ 0.5（P2，权重调整可缓解）
...
| 场景 | 模式 | 结果 | 评估 |
|---|---|---|---|
| 稳定低频（每周 1 次） | 30天内活跃5次 | false | ✅ 正确（频率=0.17 < 0.3） |
| 稳定低频（每周 2 次） | 30天内活跃8次 | false | ⚠️ 可能漏判 |
| 隔天运动 | 最大连续次数=1 | false | ✅ 连续性不足 |
| 30天活跃22天 | i%4==0 休息 | **true** | ✅ |

* *P1 风险：无领域差异。** 每周运动 2 次是合理习惯，但频率 = 8/30 = 0.27 < 0.3 被拒。需要领域因子。
...
| 领域 | 期望频率 | 当前结果 | 偏差 |
|---|---|---|---|
| 签到（每日） | 0.7-1.0 | ✅ 容易检测 | 无 |
| 专注（工作日） | 0.5-0.7 | ✅ 容易检测 | 无 |
| 阅读（高频） | 0.4-0.6 | ✅ 可检测 | 轻微 |
| 运动（每周 2-3 次） | 0.1-0.15 | ❌ 频率不足 | **严重：会漏判** |
| 英语（学习） | 0.3-0.5 | ✅ 可检测 | 无 |
...
- ✅ Zero import (does not depend on any module)
- ✅ Zero Store writing
...
`isSustained` 字段仍使用 Phase 22.1 的简单判断（`volatility < 50 && activeDays >= 5`），未升级为习惯形成检测。

This is the correct Additive First behavior: Phase 22.2 only adds modules and does not modify existing modules.
...
## workbenchDailyFeedback.test.js failed
...
- ✅ 基线提交 `232ad82` 不含 Phase 21/22 变更
...
| 窗口 | p50 | p95 | 最大 | 目标 |
|---|---|---|---|---|
| 7天 | 0.002毫秒 | 0.010毫秒 | 0.133毫秒 | <10毫秒 ✅ |
| 30天 | 0.003毫秒 | 0.005毫秒 | 0.329毫秒 | <10毫秒 ✅ |
| 90天 | 0.004毫秒 | 0.017毫秒 | 0.198毫秒 | <10毫秒 ✅ |
| 365天 | 0.012毫秒 | 0.023毫秒 | 0.101毫秒 | <10毫秒 ✅ |
...
## P1 必须在 Phase 22.3 实现前处理

When a module is about to be consumed by an AI Context / Memory candidate, the following 4 P1 issues must be resolved:

1. 添加 `currentConsecutive` 区分新旧习惯
2. Redefine `isHabitForming` semantics as "early forming signal" or raise the continuity threshold
