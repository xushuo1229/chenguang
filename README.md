# Zeno · Personal Learning Agent Operating System

> 认识自己，规划当下，持续成长。

一个面向大学生的全栈 Personal Learning Agent Operating System：每日打卡、课程进度、每日阅读、英语学习、运动记录、待办计划、专注统计，数据全平台打通，支持多设备云端同步。
...
| 层级 | 技术 | 说明 |
|------|------|------|
| 前端 | 原生 HTML/CSS/JS + Vite 6（多页构建） | index（落地/注册登录）、workbench（工作台）、stats（数据统计）、ai（Zeno Learning Agent） |
...
| 智能层 | AI 2.0「Zeno Learning Agent」 | 前端 `js/aiContext.js` 从 CGAnalytics/Goal Engine 构建只读结构化 Context（含预算裁剪与确定性洞察），后端持有 System Prompt 并经 Provider Adapter（OpenAI 兼容，默认 DeepSeek）转发 `POST /api/ai/chat`；`API Key` 只存后端 `.env`，浏览器不接触；未配置/离线时展示友好提示（应用离线优先） |
...
- **Zeno Learning Agent**：接入真实大模型对话（OpenAI 兼容国内大模型，默认 DeepSeek），把本地成长数据注入上下文给出个性化建议；未配置/离线自动回退本地模板
- **每日打卡**：一键打卡、连续天数统计
...
├── css/
│   ├── variables.css         # 设计变量（「Zeno Dawn」设计系统 token + 旧名兼容）
...
- AI 未配置（后端未设 `AI_API_KEY`）或大模型接口不可用：Zeno Learning Agent 自动回退本地模板回复，应用不中断。
